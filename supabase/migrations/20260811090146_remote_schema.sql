...
ALTER TABLE public.joueurs
  DROP CONSTRAINT email_parent_requis_si_mineur;
...
CREATE TABLE public.jetons_consentement_parental (
  id         uuid                     DEFAULT gen_random_uuid() NOT NULL,
  joueur_id  uuid                     NOT NULL,
  token      text                     NOT NULL,
  cree_le    timestamp with time zone DEFAULT now() NOT NULL,
  expire_le  timestamp with time zone DEFAULT (now() + '7 days'::interval) NOT NULL,
  utilise_le timestamp with time zone
);

ALTER TABLE public.jetons_consentement_parental
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.jetons_consentement_parental
  ADD CONSTRAINT jetons_consentement_parental_joueur_id_fkey FOREIGN KEY (joueur_id) REFERENCES public.joueurs(id) ON DELETE CASCADE;

ALTER TABLE public.jetons_consentement_parental
  ADD CONSTRAINT jetons_consentement_parental_pkey PRIMARY KEY (id);

ALTER TABLE public.jetons_consentement_parental
  ADD CONSTRAINT jetons_consentement_parental_token_key UNIQUE (token);

GRANT ALL ON public.jetons_consentement_parental TO anon;

GRANT ALL ON public.jetons_consentement_parental TO authenticated;

GRANT ALL ON public.jetons_consentement_parental TO service_role;

ALTER TABLE public.joueurs
  ADD CONSTRAINT email_parent_requis_si_actif_et_mineur CHECK (statut_acces_service <> 'actif'::text OR date_naissance <= (CURRENT_DATE - '15 years'::interval) OR email_parent IS
    NOT NULL);
