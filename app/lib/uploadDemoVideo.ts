...
const TIMEOUT_APPEL_INITIAL_MS = 10000; // évite une attente infinie sur l'appel à l'Edge Function
...
// fetch avec limite de temps -- sans ça, un réseau mort ne provoque pas
// toujours une erreur rapide, il peut simplement ne jamais répondre.
...
export async function uploaderVideoDemo(
  ...
  // Filet de sécurité global -- quoi qu'il arrive (mode avion à n'importe
  // quel moment, timeout, erreur inattendue), on retourne toujours un
  // résultat exploitable plutôt que de laisser une exception remonter
  // et bloquer l'interface indéfiniment.
  try {
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) {
